...
import { docxDiffGetRevisions, initialize } from 'docxodus';
import {
  buildIrEditScript,
  readIrDocument,
  renderIrRevisions,
...
describe('revision-list parity with the C# IR engine', () => {
  ...
  for (const [leftName, rightName] of pairs) {
    ...
    testFn(`WC ${leftName} -> ${rightName}${skipReason === null ? '' : ` [skip: ${skipReason}]`}`, async () => {
      ...
      const ts = publicRevisionShape(renderIrRevisions(buildIrEditScript(left, right), left, right));
      const cs = await docxDiffGetRevisions(leftBytes, rightBytes, {});
...
function publicRevisionShape(revisions: ReadonlyArray<IrRevision>): unknown[] {
  return revisions.map((r) => ({
    revisionType: r.type,
    text: r.text,
    author: r.author,
    date: r.date,
    moveGroupId: r.moveGroupId ?? undefined,
    isMoveSource: r.isMoveSource ?? undefined,
    formatChange: r.formatChange
      ? {
      ...
      : undefined,
    leftAnchor: r.leftAnchor ?? undefined,
    rightAnchor: r.rightAnchor ?? undefined,
  }));
}
