...
@"<w:style w:type=""character""
    ...
    <w:rPr>
      <w:vertAlign w:val=""superscript""/>
            ...
            if (element != null)
            {
                if (element.Name == W.r)
                {
                    ...
                    if (statusList.Count() > 1)
                        throw new DocxodusException("Internal error - have both deleted and inserted text elements in the same run.");
        ...
        private static void TestForInvalidContent(WordprocessingDocument wDoc)
        {
            foreach (var part in wDoc.ContentParts())
            {
                ...
                if (xDoc.Descendants(W.altChunk).Any())
                    throw new DocxodusException("Unsupported document, contains w:altChunk");
                if (xDoc.Descendants(W.subDoc).Any())
                    throw new DocxodusException("Unsupported document, contains w:subDoc");
                if (xDoc.Descendants(W.contentPart).Any())
                    throw new DocxodusException("Unsupported document, contains w:contentPart");
            }
        }
        ...
        private static List<CorrelatedSequence> Lcs(ComparisonUnit[] cu1, ComparisonUnit[] cu2, WmlComparerSettings settings)
        {
            ...
            CorrelatedSequence cs = new CorrelatedSequence()
            {
                CorrelationStatus = Docxodus.CorrelationStatus.Unknown,
            ...
            var grouped = list
                .GroupBy(sr =>
                {
                    ...
                    if (level >= sr.AncestorElements.Length)
                        throw new DocxodusException("Internal error 4 - why do we have ComparisonUnitAtom objects with fewer ancestors than its siblings?");
            ...
            var elementList = grouped
                .Select(g =>
                {
                    ...
                    if (level >= g.First().AncestorElements.Length)
                        throw new DocxodusException("Internal error 3 - why do we have ComparisonUnitAtom objects with fewer ancestors than its siblings?");
    ...
    public class ComparisonUnitAtom : ComparisonUnit
    {
        ...
        public override string ToString(int indent)
        {
            ...
            if (CorrelationStatus != Docxodus.CorrelationStatus.Nil)
    ...
    internal class ComparisonUnitGroup : ComparisonUnit
    {
        ...
        public ComparisonUnitGroup(IEnumerable<ComparisonUnit> comparisonUnitList, ComparisonUnitGroupType groupType, int level)
        {
            ...
            if (groupType == Docxodus.ComparisonUnitGroupType.Table)
                ancestorName = W.tbl;
            else if (groupType == Docxodus.ComparisonUnitGroupType.Row)
                ancestorName = W.tr;
            else if (groupType == Docxodus.ComparisonUnitGroupType.Cell)
                ancestorName = W.tc;
            else if (groupType == Docxodus.ComparisonUnitGroupType.Paragraph)
                ancestorName = W.p;
            else if (groupType == Docxodus.ComparisonUnitGroupType.Textbox)
            ...
            if (ancestor == null)
                throw new DocxodusException("Internal error: ComparisonUnitGroup");
        ...
        public static ComparisonUnitAtom GetFirstComparisonUnitAtomOfGroup(ComparisonUnit group)
        {
            ...
            while (true)
            {
                ...
                if (tw == null)
                    throw new DocxodusException("Internal error: GetFirstComparisonUnitAtomOfGroup");
    ...
    class CorrelatedSequence
    {
        ...
        public override string ToString()
        {
            ...
            if (CorrelationStatus == Docxodus.CorrelationStatus.Equal)
            {
